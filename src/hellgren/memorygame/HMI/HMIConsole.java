...
public class HMIConsole implements HMI { //This class is the interpretor between player and game
    private Scanner input;   //input variable

    public HMIConsole() {  //constructor
    ...
    public void showPlayfield(GameStatus status, GameSetup setup) {
        ...
        for (int ri = 1; ri <= setup.getnRows(); ri++) { //ri corresponds to row
            ...
            {
                pos.setRiCi(ri, ci);
                keypos = GameManager.getKeypos(pos); //call static function in super class
    ...
    public void setKeysofPlayerchosenPos(GameStatus status, GameSetup setup) {
        ...
        poskeylist.clear();
        pos = readCardPos(setup, "first");  poskeylist.add(GameManager.getKeypos(pos));
        pos = readCardPos(setup, "second"); poskeylist.add(GameManager.getKeypos(pos));
    }
}
